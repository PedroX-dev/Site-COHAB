document.addEventListener('DOMContentLoaded', () => {
  ...
  const redesMenu = redes?.querySelector('.redes-menu');

  const abrirRedes = () => {
    if (!redes) return;
    redes.classList.add('aberto');
    document.body.classList.add('redes-open');
    toggle?.setAttribute('aria-expanded', 'true');
    redesMenu?.setAttribute('aria-hidden', 'false');
  };

  const fecharRedes = () => {
    if (!redes) return;
    redes.classList.remove('aberto');
    document.body.classList.remove('redes-open');
    toggle?.setAttribute('aria-expanded', 'false');
    redesMenu?.setAttribute('aria-hidden', 'true');
  };

  toggle?.addEventListener('click', (e) => {
    e.stopPropagation();
    if (redes?.classList.contains('aberto')) {
      fecharRedes();
      return;
    }
    abrirRedes();
  ...
  redesMenu?.addEventListener('click', (e) => {
    e.stopPropagation();
  });

  document.addEventListener('click', (e) => {
    if (!redes || redes.contains(e.target)) return;
    fecharRedes();
  });
...
const isQuemEquem = pathLower.includes('quemequem.html') || pathLower.includes('conselhos.html');
  ...
  if (buscaForm && buscaInput) {
    ...
    if (menuSearch) {
      ...
      document.addEventListener('click', (event) => {
        if (menuSearch.classList.contains('menu-search-open') && !menuSearch.contains(event.target)) {
          closeSearch();
        }
      });
    }
  ...
  // Breadcrumbs
  const breadcrumbMenu = document.querySelector('nav.menu');
  ...
  if (breadcrumbMenu && breadcrumbMain && breadcrumbIsMenu && !document.querySelector('.breadcrumbs')) {
    ...
    const linkData = Array.from(breadcrumbMenu.querySelectorAll('a[href]'))
      .map((link) => {
        const href = link.getAttribute('href') || '';
        if (!href || href === '#') return null;
        const url = new URL(href, window.location.href);
        const section = link.closest('.menu-item')?.querySelector(':scope > a')?.textContent?.trim() || '';
        return {
    ...
    const inner = breadcrumbMenu.querySelector('.menu-inner');
    const menuSearch = breadcrumbMenu.querySelector('.menu-search');

    if (menuSearch) {
      const bar = document.createElement('div');
      bar.className = 'breadcrumbs-bar';
      bar.appendChild(nav);
      bar.appendChild(menuSearch);
      (inner || breadcrumbMenu).insertAdjacentElement('afterend', bar);
    } else {
      (inner || breadcrumbMenu).insertAdjacentElement('afterend', nav);
    }
  ...
  (function () {
        ...
        if (!lastTarget && !historyTarget) return;
