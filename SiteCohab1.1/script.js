...
const isQuemEquem = pathLower.includes('quemequem.html') || pathLower.includes('conselhos.html');
  ...
  // Acessos rapidos
  (function () {
    const items = document.querySelectorAll('.acesso-rapido');
    ...
    const updateContainerState = () => {
      const hasOpen = Array.from(items).some((item) => item.classList.contains('is-open'));
    ...
    const scheduleClose = (item) => {
      ...
      const timer = setTimeout(() => {
        if (item.matches(':hover') || item.matches(':focus-visible') || item.matches(':focus')) {
          scheduleClose(item);
          return;
        }
    ...
    document.addEventListener('pointerdown', (event) => {
      if (!container.classList.contains('has-open')) {
        return;
      }
