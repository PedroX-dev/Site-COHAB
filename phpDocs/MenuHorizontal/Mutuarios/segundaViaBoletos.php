                ...
                <nav class="header-menu header-menu-right" aria-label="Menu principal direito">
                    <ul class="header-menu-list">
                        <li class="header-menu-item">
                            ...
                            <ul class="submenu">
                            <li><a href="../../MenuHorizontal/Mutuarios/segundaViaBoletos.php">2ª Via de Boletos</a></li>
...
<main class="centralizarX">
        <span id="titulo">2ª Via de Boletos</span>
        <hr>
        <p>
            Solicite a segunda via do seu boleto pelo site da Cohab Minas.<br>
            <b>Informe o número do contrato ou o CPF do titular e um celular válido para o envio via WhatsApp.</b>
            <br><br>
            <i>Os dados são utilizados apenas para emissão e entrega do boleto.</i>
        </p>
        <div class="stepper" id="boleto-stepper">
            <div class="stepper-nav" aria-label="Navegação entre etapas">
                <button type="button" class="stepper-btn" id="step-prev" aria-label="Voltar etapa" disabled>
                    <i class="bi bi-chevron-left" aria-hidden="true"></i>
                </button>
                <div class="stepper-title" id="step-title">Identificação do contrato</div>
                <button type="button" class="stepper-btn" id="step-next" aria-label="Avançar etapa" disabled>
                    <i class="bi bi-chevron-right" aria-hidden="true"></i>
                </button>
            </div>
            <div class="emissao-overlay" id="emissao-overlay" aria-hidden="true"></div>
            <div id="boleto-status" class="cpf-card hidden stepper-status" role="status" aria-live="polite">
                <span class="cpf-note" id="boleto-status-text"></span>
            </div>
            <div class="stepper-body">
        <form id="boleto-form" class="cadastro-form step-panel is-active" data-step="0" data-title="Identificação do contrato" action="#" method="post" novalidate>
            <div class="form-section">
                <h3>Identificação do contrato</h3>
                <div class="form-grid">
                    <label>Forma de acesso *</label>
                    <div class="radio-group">
                        <label><input type="radio" name="tipo-acesso" value="contrato" checked> Número do contrato</label>
                        <label><input type="radio" name="tipo-acesso" value="cpf"> CPF do titular</label>
                    </div>

                    <label for="contrato" id="label-contrato">Número do contrato *</label>
                    <input id="contrato" name="contrato" type="text" inputmode="numeric" autocomplete="off" placeholder="Somente números" maxlength="24" required>

                    <label for="cpf" id="label-cpf" class="hidden">CPF do titular *</label>
                    <input id="cpf" name="cpf" type="text" inputmode="numeric" autocomplete="off" placeholder="000.000.000-00" maxlength="14" class="hidden">
                </div>
                <span class="cpf-note">Preencha apenas números. Não é necessário usar pontos ou traços.</span>
            </div>

            <div class="form-section">
                <h3>Contato do mutuário</h3>
                <div class="form-grid">
                    <label for="celular">Celular (WhatsApp) *</label>
                    <input id="celular" name="celular" type="tel" inputmode="tel" autocomplete="tel" placeholder="(00) 00000-0000" maxlength="15" required>
                </div>
                <span class="cpf-note">Este número será usado apenas para enviar o boleto via WhatsApp.</span>
            </div>
            <div class="form-section hidden" id="emissao-section">
                <h3>Tipo de emissão</h3>
                <div class="radio-group" id="emissao-opcoes"></div>
                <span class="cpf-note" id="emissao-nota"></span>
                <div class="emissao-actions">
                    <button type="button" class="btn-submit" id="emissao-confirmar">Continuar</button>
                    <button type="button" class="btn-reserva" id="emissao-cancelar">Cancelar</button>
                </div>
            </div>

            <button class="btn-submit centralizarX" type="submit">Continuar</button>
        </form>

        <div id="contratos-lista" class="cadastro-form step-panel step-locked" data-step="1" data-title="Seleção de contrato" aria-live="polite">
            <div class="form-section">
                <h3>Selecione um contrato</h3>
                <div class="form-grid" id="contratos-opcoes"></div>
            </div>
        </div>

        <div id="mutuario-dados" class="cadastro-form step-panel step-locked" data-step="2" data-title="Dados cadastrais" aria-live="polite">
            <div class="form-section">
                <h3>Dados cadastrais</h3>
                <div class="form-grid">
                    <label for="dados-contrato">Contrato</label>
                    <input id="dados-contrato" type="text" readonly>

                    <label for="dados-nome">Nome</label>
                    <input id="dados-nome" type="text" readonly>

                    <label for="dados-nascimento">Data de Nascimento</label>
                    <input id="dados-nascimento" type="text" readonly>

                    <label for="dados-tipo">Tipo de Pessoa</label>
                    <input id="dados-tipo" type="text" readonly>

                    <label for="dados-cpf">CNPJ/CPF</label>
                    <input id="dados-cpf" type="text" readonly>

                    <label for="dados-vencimento">Vencimento</label>
                    <input id="dados-vencimento" type="text" readonly>

                    <label for="dados-endereco">Endereço</label>
                    <input id="dados-endereco" type="text" readonly>

                    <label for="dados-email">E-mail</label>
                    <input id="dados-email" type="text" readonly>
                </div>
                <div class="cadastro-reserva">
                    <button class="btn-reserva" type="button" id="btn-informe">Informe de rendimento</button>
                </div>
            </div>
        </div>

        <form id="prestacoes-form" class="cadastro-form step-panel step-locked" data-step="3" data-title="Prestações do mutuário" action="#" method="post" novalidate>
            <div class="form-section">
                <h3>Prestações do mutuário</h3>
                <div class="table-responsive">
                    <table class="table table-striped table-prestacoes">
                        <thead>
                            <tr>
                                <th><input type="checkbox" id="prestacoes-todos"></th>
                                <th>Prestação</th>
                                <th>Vencimento</th>
                                <th>Encargo</th>
                                <th class="d-none d-lg-table-cell">FGTS</th>
                                <th class="d-none d-md-table-cell">Mora</th>
                                <th class="d-none d-lg-table-cell">Acrés/Abat</th>
                                <th class="d-none d-md-table-cell">Atualização</th>
                                <th class="d-none d-md-table-cell">Juros Rem</th>
                                <th class="d-none d-md-table-cell">Multa</th>
                                <th>Total</th>
                            </tr>
                        </thead>
                        <tbody id="prestacoes-body">
                            <tr>
                                <td colspan="11" class="text-center text-muted">Nenhuma prestação carregada.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div class="cadastro-reserva">
                    <button class="btn-submit" id="btn-gerar-boleto" type="submit" disabled>
                        Gerar boleto <strong id="val-tot">R$ 0,00</strong>
                    </button>
                </div>
                <input type="hidden" name="contrato" id="prestacoes-contrato">
                <input type="hidden" name="acordo" id="prestacoes-acordo">
            </div>
        </form>

        <div id="boleto-result" class="cadastro-form step-panel step-locked" data-step="4" data-title="Boleto gerado" aria-live="polite">
            <div class="form-section">
                <h3>Boleto gerado</h3>
                <p class="cpf-note" id="boleto-resumo">Escolha como deseja receber o boleto.</p>
                <div class="cpf-card" id="linha-digitavel-box">
                    <div class="cpf-row">
                        <input type="text" class="cpf-input" id="linha-digitavel" readonly>
                        <button type="button" class="cpf-button" id="btn-copiar-linha">Copiar</button>
                    </div>
                    <span class="cpf-note">Use o código de barras ou a linha digitável para pagamento.</span>
                </div>
                <div class="cadastro-reserva">
                    <button class="btn-reserva" type="button" id="btn-whatsapp">Enviar por WhatsApp</button>
                    <button class="btn-reserva" type="button" id="btn-download">Baixar boleto</button>
                    <button class="btn-reserva" type="button" id="btn-print">Imprimir boleto</button>
                    <button class="btn-reserva" type="button" id="btn-inline">Exibir na página</button>
                </div>
            </div>
            <div class="form-section hidden" id="boleto-visualizacao">
                <h3>Visualização do boleto</h3>
                <div class="termo-texto">
                    <iframe id="boleto-pdf" name="boleto-pdf" title="Boleto" width="100%" height="720"></iframe>
                </div>
            </div>
        </div>
            </div>
