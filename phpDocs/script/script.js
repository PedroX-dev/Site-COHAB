document.addEventListener('DOMContentLoaded', () => {
  ...
  const getSiteRoot = () => {
    ...
    const pagesMarker = '/paginas/';
    const idx = lower.indexOf(marker);
    const pagesIdx = lower.indexOf(pagesMarker);
    const rootPath = idx !== -1
      ? path.slice(0, idx + 1)
      : (pagesIdx !== -1
        ? path.slice(0, pagesIdx + 1)
  ...
  const ORIG_PAGES = {
    mutuarios: 'paginas/paginaMutuarios.php',
    prefeituras: 'paginas/paginaPrefeituras.php',
    inicial: 'paginas/paginaInicial.php',
  };
  ...
  const inferOrigemFromPath = () => {
    ...
    if (path.endsWith('/paginamutuarios.php')) return 'mutuarios';
    if (path.endsWith('/paginaprefeituras.php')) return 'prefeituras';
    if (path.endsWith('/paginainicial.php')) return 'inicial';
    return '';
  };
...
const cadastroForm = document.querySelector('.cadastro-form');
  ...
  // Formulario de automacao: popup para descricao detalhada
  const descricaoField = document.getElementById('descricao');
  const descricaoBackdrop = document.getElementById('descricao-backdrop');

  if (descricaoField && descricaoBackdrop) {
    const abrirDescricaoPopup = () => {
      if (document.body.classList.contains('descricao-popup-aberta')) return;
      document.body.classList.add('descricao-popup-aberta');
      descricaoBackdrop.hidden = false;
      setTimeout(() => descricaoField.focus(), 0);
    };

    const fecharDescricaoPopup = () => {
      document.body.classList.remove('descricao-popup-aberta');
      descricaoBackdrop.hidden = true;
      descricaoField.blur();
    };

    descricaoField.addEventListener('focus', abrirDescricaoPopup);
    descricaoField.addEventListener('click', abrirDescricaoPopup);
    descricaoBackdrop.addEventListener('click', fecharDescricaoPopup);
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Enter' && e.target === descricaoField) {
        e.preventDefault();
        fecharDescricaoPopup();
      }
    });
  ...
  // Formulario de automacao: anexos (pelo menos um arquivo entre fotos ou videos)
  ...
  if (cadastroForm && imagensInput && videosInput) {
    const possuiArquivos = (input) => input?.files && input.files.length > 0;

    const validarAnexos = () => {
      const ok = possuiArquivos(imagensInput) || possuiArquivos(videosInput);
      const mensagem = ok ? '' : 'Anexe pelo menos um arquivo (fotos ou vídeo).';
      imagensInput.setCustomValidity(mensagem);
      videosInput.setCustomValidity(mensagem);
      return ok;
    };

    const formatarListaArquivos = (input) => {
      if (!input?.files || input.files.length === 0) return 'Nenhum arquivo selecionado';
      if (input.files.length === 1) return input.files[0].name;
      return `${input.files.length} arquivos selecionados`;
    };

    const atualizarTextoArquivos = () => {
      if (imagensFiles) imagensFiles.textContent = formatarListaArquivos(imagensInput);
      if (videosFiles) videosFiles.textContent = formatarListaArquivos(videosInput);
    };

    imagensInput.addEventListener('change', validarAnexos);
    videosInput.addEventListener('change', validarAnexos);
    imagensInput.addEventListener('change', atualizarTextoArquivos);
    videosInput.addEventListener('change', atualizarTextoArquivos);
    ...
    atualizarTextoArquivos();
  }

  // Formulario de automacao: lembrar dados
  const lembrarDados = document.getElementById('lembrar-dados');
  const camposLembrar = {
  ...
  if (lembrarDados && Object.values(camposLembrar).every(Boolean)) {
    const storageKey = 'form_automacao_lembrar';
    const carregarDados = () => {
      try {
        const raw = localStorage.getItem(storageKey);
        if (!raw) return;
        const dados = JSON.parse(raw);
        Object.entries(camposLembrar).forEach(([key, input]) => {
          if (dados[key] && input) input.value = dados[key];
        });
    ...
    const salvarDados = () => {
      if (!lembrarDados.checked) return;
      const payload = {};
      Object.entries(camposLembrar).forEach(([key, input]) => {
    ...
    lembrarDados.addEventListener('change', () => {
    ...
    Object.values(camposLembrar).forEach((input) => {
  ...
  // Formulario de automacao: mascara de telefone
  const aplicarMascaraTelefone = (input) => {
    ...
    input.value = `(${ddd}) ${resto.slice(0, 5)}-${resto.slice(5, 9)}`;
  };

  const celularInput = document.getElementById('celular');
  const telefoneInput = document.getElementById('telefone');

  [celularInput, telefoneInput].forEach((input) => {
    if (!input) return;
    input.addEventListener('input', () => aplicarMascaraTelefone(input));
    input.addEventListener('blur', () => aplicarMascaraTelefone(input));
  });
  
  // Quem e quem / Conselhos (popup)
const pathLower = window.location.pathname.toLowerCase();
const isQuemEquem = pathLower.includes('quemequem.php') || pathLower.includes('conselhos.php');
  ...
  const buscaInput = buscaForm?.querySelector('input[name="q"]');
    ...
    const searchPageUrl = new URL('paginas/pesquisa.php', siteRoot).toString();

    const SEARCH_PAGES = [
      'paginas/paginaInicial.php',
      'MenuHorizontal/Transparencia/remuneracao.php',
      'MenuHorizontal/Transparencia/prestacaoFEH.php',
      'MenuHorizontal/Transparencia/prestacaoCohab.php',
      'MenuHorizontal/Transparencia/informacoes.php',
      'MenuHorizontal/Transparencia/documentos.php',
      'MenuHorizontal/Transparencia/dados.php',
      'MenuHorizontal/Mutuarios/transferenciaFincan.php',
      'MenuHorizontal/Mutuarios/seguros.php',
      'MenuHorizontal/Mutuarios/plantas.php',
      'MenuHorizontal/Mutuarios/manual.php',
      'MenuHorizontal/Mutuarios/liquidacao.php',
      'MenuHorizontal/Mutuarios/escrituras.php',
      'MenuHorizontal/Mutuarios/segundaViaBoletos.php',
      'MenuHorizontal/Mutuarios/contrato.php',
      'MenuHorizontal/Mutuarios/bonus.php',
      'MenuHorizontal/Atendimento/perguntas.php',
      'MenuHorizontal/Atendimento/ouvidoria.php',
      'MenuHorizontal/Atendimento/imoveis.php',
      'MenuHorizontal/Licitacoes/licitacoes.php',
      'MenuHorizontal/Licitacoes/dataroom.php',
      'MenuHorizontal/Licitacoes/contratos.php',
      'MenuHorizontal/Atendimento/faleConosco.php',
      'MenuHorizontal/Atendimento/comoParticipar.php',
      'MenuHorizontal/Atendimento/cohabMaisPerto.php',
      'MenuHorizontal/Atendimento/cadastro.php',
      'MenuHorizontal/Institucional/quemEquem.php',
      'MenuHorizontal/Institucional/planejamento.php',
      'MenuHorizontal/Institucional/noticias.php',
      'MenuHorizontal/Institucional/legislacao.php',
      'MenuHorizontal/Institucional/conselhos.php',
      'MenuHorizontal/Institucional/historia.php',
      'MenuHorizontal/Institucional/estrutura.php',
      'MenuHorizontal/Governanca/relatorioAUD.php',
      'MenuHorizontal/Governanca/etica.php',
      'MenuHorizontal/Governanca/relatorioADM.php',
      'MenuHorizontal/Governanca/estrategia.php',
      'MenuHorizontal/Governanca/lgpd.php',
      'MenuHorizontal/Governanca/dividendos.php',
      'MenuHorizontal/Governanca/integridade.php',
      'MenuHorizontal/Governanca/cartaAnual.php',
      'MenuHorizontal/Institucional/informacoes.php',
      'MenuHorizontal/Governanca/acionistas.php'
    ];
    ...
    buscaForm.addEventListener('submit', (event) => {
      ...
      const isSearchPage = window.location.pathname.toLowerCase().includes('pesquisa.php');
  ...
  if (breadcrumbHeader && breadcrumbMain && breadcrumbIsMenu && !document.querySelector('.breadcrumbs')) {
    ...
    const siteRoot = getSiteRoot();
    const homeUrl = new URL('paginas/paginaInicial.php', siteRoot).toString();
...
(() => {
  ...
  const fetchJson = async (url, options) => {
    ...
    if (!response.ok) {
      const mensagem = payload?.message || `Falha na resposta da API (${response.status}).`;
  ...
  const criarTokenWhatsApp = async () => {
    const payload = {
    ...
    return fetchJson('../../php/whatsapp/token.php', {
      method: 'POST',
      headers: {
  ...
  if (whatsappBtn) {
    whatsappBtn.addEventListener('click', async () => {
      ...
      try {
        mostrarStatus('Gerando codigo para WhatsApp...');
        const resposta = await criarTokenWhatsApp();
        const url = resposta?.whatsapp_url || '';
        if (!url) {
          throw new Error('URL de WhatsApp nao retornada.');
        }
        ...
        mostrarStatus('WhatsApp aberto com a mensagem pronta. Basta enviar.');
      } catch (erro) {
        if (popup && !popup.closed) {
          popup.close();
        }
        mostrarStatus(erro?.message || 'Nao foi possivel gerar o codigo do WhatsApp.');
      }
    });
  }
  ...
  (function () {
        ...
        var dataUrl = new URL('dados/atualizacoes.json', siteRoot).toString();
