...
const isQuemEquem = pathLower.includes('quemequem.html') || pathLower.includes('conselhos.html');
  ...
  if (buscaForm && buscaInput) {
    ...
    if (menuSearch) {
      ...
      const isBasePage = path.endsWith('/paginainicial.html') || path.endsWith('paginainicial.html');

      if (!isBasePage) {
        const searchButton = menuSearch.querySelector('button[type="submit"], button');
        const openSearch = () => menuSearch.classList.add('menu-search-open');
        const closeSearch = () => menuSearch.classList.remove('menu-search-open');
